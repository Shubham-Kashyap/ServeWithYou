...
<section class="content-header">
      <h1>
        All Jobs
        <small>list of all registered jobs</small>
      </h1>
      <ol class="breadcrumb">
            ...
            <div class="box-body ">
              <div class="table-responsive">
                ...
                <table id="jobs" class="table display table-bordered table-striped" style="width:100%;">
                  ...
                  <tbody>
                    @foreach($data as $row)
                    @if($row->job_status === 'accepted' and $row->job_done_status === 'pending' )
                    <tr>
                        <td>{{ $row->id }}</td>
                        ...
                        <td>
                            @if($row->job_status == 'accepted')
                                <span class="label label-xs bg-navy">Accepted</span>
                            @endif
                            @if($row->job_status == 'rejected')
                                <span class="label label-xs bg-info">Rejected</span>
                            @endif
                        ...
                        <td>
                        <!-- <a href="{{ URL::to('subadmin/jobs/edit-job/'.$row->id) }}" class="btn btn-xs bg-navy" >Edit</a>
                        <a href="{{ URL::to('subadmin/jobs/view-job/'.$row->id) }}" class="btn btn-xs btn-success" >View</a>
                        <a type="button" data-toggle="modal" data-target="#{{ 'deleteJob-'.$row->id }}"  class="btn btn-xs btn-danger" >Delete</a> -->
                        
                        <div class="btn-group" style="width:80px;">
                        @if(Auth::guard('subadmin')->user()->edit_job_permission == 'on')
                          <a href="{{ URL::to('subadmin/jobs/edit-job/'.$row->id) }}" data-toggle="tooltip" data-placement="top" title="Edit" class="btn btn-xs bg-navy"><i class="fa fa-pencil "></i></a>
                        @endif
                          <a href="{{ URL::to('subadmin/jobs/view-job/'.$row->id) }}" data-toggle="tooltip" data-placement="top" title="View" class="btn btn-success btn-xs"><i class="fa  fa-eye"></i></a>
                          @if(Auth::guard('subadmin')->user()->delete_job_permission == 'on')
                        ...
                        <!-- <a type="button" data-toggle="modal" data-target="#{{ 'deleteJob-'.$row->id }}" class="btn btn-xs btn-danger x"><i class="fa fa-trash-o"></i></a> -->
                          ...
                          <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                              ...
                              <div class="modal-footer">
                                ...
                                <a href="{{ URL::to('subadmin/jobs/delete-job/'.$row->id) }}" class="btn bg-navy">Delete Job</a>
